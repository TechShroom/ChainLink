...
/**
 * A container object which may or may not contain a {@code boolean} value. If a
 * value is present, {@code isPresent()} will return {@code true} and
 * {@code getAsBoolean()} will return the value.
 *
 * <p>
 * Additional methods that depend on the presence or absence of a contained
 * value are provided, such as {@link #orElse(boolean) orElse()} (return a
 * default value if value not present) and
 * {@link #ifPresent(java.util.function.) ifPresent()} (execute a block of code
...
public final class OptionalBoolean {
     ...
     *
     * @implNote generally only one empty instance,
     ...
     * for this OptionalBoolean.
     *
     * @apiNote Though it may be tempting to do so, avoid testing if an object
     *          is empty by comparing with {@code ==} against instances returned
     *          by {@code OptionalBoolean.empty()}. There is no guarantee that
     ...
     *
     * @param other
     *            a {@code BooleanSupplier} whose result is returned if no value
     *            is present
     * @return the value if present otherwise the result of
     *         {@code other.getAsBoolean()}
     * @throws NullPointerException
     ...
     * <li>the present values are "equal to" each other
     * </ul>
     *
     * @param obj
     ...
     *           representation in the result. Empty and present instances must
